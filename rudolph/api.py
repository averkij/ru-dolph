...
class ruDolphApi:
    ...
    def __init__(
        ...
        *,
        ...
        self.text_vocab_size = (
            self.vocab_size - self.l_text_seq_length - self.text_special_tokens
        )
    ...
    def dream(
        ...
        ppl_txt, ppl_img, scores = self.self_reranking_by_text(
            text, codebooks, bs=bs, seed=seed, ppl_txt_w=ppl_txt_w
        )
        result = []
        with torch.no_grad():
        ...
        for chunk in more_itertools.chunked(range(images_num), bs):
            ...
            with torch.no_grad():
                ...
                if image_prompts is not None:
                    prompts_idx, prompts = (
                        image_prompts.image_prompts_idx,
                        image_prompts.image_prompts,
                    )
                    prompts = prompts.repeat(chunk_bs, 1)
                iter_range = range(
                    self.l_text_seq_length,
                    self.l_text_seq_length + self.image_seq_length,
                )
                if not self.quite:
                ...
                for idx in iter_range:
                    ...
                    if image_prompts is not None and idx in prompts_idx:
                        ...
                        logits /= temperature
                        filtered_logits = transformers.top_k_top_p_filtering(
    ...
    def self_reranking_by_text(
        ...
        r_encoded[torch.where(r_encoded == self.spc_id)] = self.spc_tokens[
            r_special_token
        ]

        l_encoded = self.encode_text(l_template, text_seq_length=self.l_text_seq_length)
        l_encoded[torch.where(l_encoded == self.spc_id)] = self.spc_tokens[
            l_special_token
        ]
        ...
        for chunk in more_itertools.chunked(codebooks, bs):
            ...
            with torch.no_grad():
                ...
                image_logits = logits[
                    :,
                    self.vocab_size :,
                    self.l_text_seq_length : self.l_text_seq_length
                ...
                r_text_logits = (
                    logits[
                        :,
                        : self.vocab_size - self.l_text_seq_length,
                        -self.r_text_seq_length : -1,
                    ]
                    .contiguous()
                    .float()
                )
        ...
        scores = (ppl_txt - ppl_txt.min()) / (
            ppl_txt.quantile(q=self.q) - ppl_txt.min()
        ) * ppl_txt_w + (ppl_img - ppl_img.min()) / (
            ppl_img.quantile(q=self.q) - ppl_img.min()
        ) * (
            1 - ppl_txt_w
        )
    ...
    def zs_clf(
        ...
        for _class in classes:
            ...
            r_encoded.append(
                self.encode_text(r_text, text_seq_length=self.r_text_seq_length)
            )
            l_text = l_template.format(_class).lower().strip()
            l_encoded.append(
                self.encode_text(l_text, text_seq_length=self.l_text_seq_length)
            )
        ...
        with torch.no_grad():
            ...
            image_input_ids = self.vae.get_codebook_indices(
                images, disable_gumbel_softmax=True
            )
            cache = None
            ...
            for indexes in more_itertools.chunked(range(len(classes)), bs):
                ...
                r_text_logits = logits[
                    :,
                    : self.vocab_size - self.l_text_seq_length,
                    -self.r_text_seq_length : -1,
                ].contiguous()

                chunk_ppl_txt = self.ce_to_ppl(
                    F.cross_entropy(
                        r_text_logits[:, :, :],
                        input_ids[:, -(self.r_text_seq_length - 1) :],
                        ignore_index=0,
    ...
    def generate_texts(
        ...
        template_encoded = self.encode_text(
            template, text_seq_length=self.l_text_seq_length
        )
        ...
        template_encoded[
            torch.where(template_encoded == self.spc_id)
        ] = self.spc_tokens[special_token]
        ...
        for chunk in more_itertools.chunked(range(texts_num), bs):
            ...
            with torch.no_grad():
                ...
                cache = None
                iter_range = range(
                    template_size, min(early_stop, self.l_text_seq_length)
                )
                if not self.quite:
                    iter_range = tqdm(iter_range)
                for _ in iter_range:
            ...
            with torch.no_grad():
                ...
                for text, _ in chunk:
                    ...
                    encoded = self.encode_text(
                        text, text_seq_length=self.l_text_seq_length
                    )
                    chunk_encoded.append(encoded)

                chunk_encoded = torch.stack(chunk_encoded)
                chunk_encoded[
                    torch.where(chunk_encoded == self.spc_id)
                ] = self.spc_tokens[special_token]
    ...
    def generate_captions(
        ...
        r_encoded = self.encode_text(
            r_template.lower().strip(), text_seq_length=self.r_text_seq_length
        )
        r_encoded[torch.where(r_encoded == self.spc_id)] = self.spc_tokens[
            r_special_token
        ]
        ...
        for chunk in more_itertools.chunked(range(captions_num), bs):
            ...
            with torch.no_grad():
                ...
                iter_range = range(
                    ...
                    min(
                        self.l_text_seq_length + self.image_seq_length + early_stop,
                        self.total_seq_length,
                    ),
                )
                if not self.quite:
                ...
                generated_tokens.append(
                    out[:, self.l_text_seq_length + self.image_seq_length :]
                )
        ...
        for chunk in more_itertools.chunked(texts, bs):
            ...
            with torch.no_grad():
                ...
                for text in chunk:
                    ...
                    encoded[torch.where(encoded == self.spc_id)] = self.spc_tokens[
                        l_special_token
                    ]
                    chunk_encoded.append(encoded)
    ...
    def generate_codebooks_cfg(
        ...
        encoded_CFG = self.encode_text(
            template.format(text=text), text_seq_length=self.l_text_seq_length
        )
        encoded_CFG[torch.where(encoded_CFG == self.spc_id)] = self.spc_tokens[
            special_token
        ]
                    ...
                    if image_prompts is not None and idx in prompts_idx:
                        ...
                        for d, weight in enumerate(weights):
                            ...
                            if self.image_special_tokens:
                                logits = logits[:, : -self.image_special_tokens]
                            logits /= temperature
                            probs = transformers.top_k_top_p_filtering(
                                logits, top_k=top_k, top_p=top_p
                            )
                            if not late_softmax:
                        ...
                        out = torch.stack(
                            [
                                torch.cat((out[i :: len(weights)], sample), dim=-1)
    ...
    def show(pil_images, nrow=4, size=11, save_dir=None, show=True):
        ...
        imgs = torchvision.utils.make_grid(
            utils.pil_list_to_torch_tensors(pil_images), nrow=nrow
        )
    ...
    def get_attention_mask(self, bs):
        return torch.tril(
            torch.ones(
                (bs, 1, self.total_seq_length, self.total_seq_length),
                device=self.device,
            )
        )
    ...
    def encode_text(self, text, text_seq_length):
        tokens = self.tokenizer.tokenizer.encode(
            [text], output_type=yttm.OutputType.ID
        )[0]
    ...
    def decode_text(self, encoded):
        return self.tokenizer.tokenizer.decode(
            encoded.cpu().numpy().tolist(), ignore_ids=self.ignore_ids
        )[0]
