...
class ruDolphModel(torch.nn.Module):
    def __init__(
        ...
        self.l_text_pos_embeddings = torch.nn.Embedding(
            l_text_seq_length + 1, hidden_size
        )
        self.r_text_pos_embeddings = torch.nn.Embedding(
            r_text_seq_length + 1, hidden_size
        )
        self.image_row_embeddings = torch.nn.Embedding(
            image_tokens_per_dim, hidden_size
        )
    ...
    def forward(
        ...
        l_text_pos = self.l_text_pos_embeddings(
            torch.arange(l_text.shape[1], device=device)
        )
        ...
        if use_image:
            image_input_ids = input_ids[
                :,
                self.l_text_seq_length : self.l_text_seq_length + self.image_seq_length,
            ]
            img_pos = self.get_image_pos_embeddings(
                image_input_ids, past_length=0, device=device
            )
        ...
        if use_r_text:
            ...
            r_text_pos = self.r_text_pos_embeddings(
                torch.arange(r_text.shape[1], device=device)
            )
        ...
        l_text_logits = (
            logits[
                :,
                : self.vocab_size,
                : self.l_text_seq_length if use_image else self.l_text_seq_length - 1,
            ]
            .contiguous()
            .float()
        )
        ...
        if use_image:
            ...
            a, b = (
                ...
                self.l_text_seq_length + self.image_seq_length - 1,
            )
            image_logits = logits[:, self.vocab_size :, a:b].contiguous().float()
        if use_r_text:
            r_text_logits = (
                logits[:, : self.vocab_size, -self.r_text_seq_length : -1]
                .contiguous()
                .float()
            )
        ...
        loss_l_text = F.cross_entropy(
            l_text_logits, labels[:, : self.l_text_seq_length]
        )
