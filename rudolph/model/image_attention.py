...
def _init_mask(l_text_tokens, image_tokens_per_dim, r_text_tokens, is_bool_mask=False):
    ...
    mask = torch.tril(
        torch.ones(
            attn_size, attn_size, dtype=torch.bool if is_bool_mask else torch.float32
        )
    )
    return mask
...
def get_row_mask(
    l_text_tokens, image_tokens_per_dim, r_text_tokens, is_bool_mask=False
):
...
def get_conv_mask(
    l_text_tokens, image_tokens_per_dim, r_text_tokens, kernel=11, is_bool_mask=False
):
    mask = _init_mask(l_text_tokens, image_tokens_per_dim, r_text_tokens, is_bool_mask)
    shift = kernel // 2
    image_end = l_text_tokens + image_tokens_per_dim**2
